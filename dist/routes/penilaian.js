...
penilaian.post("/:submission_id/:juri_id", authmiddleware, authjuri, async (c) => {
    try {
        ...
        if (existing) {
            return c.json({ status: "error", message: "Submission ini sudah Anda nilai." }, 400);
        }
        // buat penilaian baru
...
penilaian.get("/daftarnilai/:lombaID", authmiddleware, authpeserta, async (c) => {
    try {
        const { lombaID } = c.req.param();
        if (!lombaID) {
            return c.json({ status: "error", message: "Lomba tidak ditemukan." }, 404);
        }
        ...
        if (!nilaiLomba || nilaiLomba.length === 0) {
            return c.json({ status: "error", message: "Belum ada penilaian untuk lomba ini." }, 404);
        }
        // Format response
