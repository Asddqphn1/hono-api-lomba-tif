...
daftarlomba.post("/", authmiddleware, authadmin, async (c) => {
    try {
        const { id, nama, tanggal, lokasi, url, // Pastikan ini sesuai dengan yang dikirim frontend
        bataswaktu, deskripsi, jenis_lomba, jumlah_anggota, } = await c.req.json();
        // Validasi jenis lomba
        if (!["INDIVIDU", "TIM"].includes(jenis_lomba)) {
            return c.json({ status: "error", message: "Jenis lomba tidak valid" }, 400);
        }
